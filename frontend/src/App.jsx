      ...
      <div className='moderustic-12345 w-full h-screen flex flex-col'>
        ...
        <div className='flex justify-center my-6'>
          <form onSubmit={handleSubmit} className='flex justify-evenly gap-3'>
            ...
            />
            <button type="submit"><IoIosAddCircle className='text-5xl' /></button>
          </form>
        </div>
        <ul className='todos flex justify-center flex-col items-center'>
          {
            todos?.map(todo => {
              return <Todo key={todo._id} _id={todo._id} value={todo.value} editTodo={() => editTodo(todo._id)} deleteTodo={() => deleteTodo(todo._id)} />
            })
          }
        </ul>
      </div>
